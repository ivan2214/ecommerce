    ...
    <form onSubmit={handleSubmit}>
      <div className="space-y-6">
            ...
            {addresses.length > 0 ? (
              <RadioGroup value={selectedAddressId} onValueChange={setSelectedAddressId} className="space-y-4">
                {addresses.map((address) => (
                  <div key={address.id} className="flex items-start space-x-3 rounded-md border p-3">
              ...
              <div className="text-center py-4">
                ...
                <Button variant="outline" className="mt-2" onClick={() => router.push("/profile?tab=addresses")}>
                  Add New Address
                </Button>
              </div>
            )}
          ...
          <CardContent>
            <RadioGroup value={paymentMethod} onValueChange={setPaymentMethod} className="space-y-4">
              ...
              <div className="flex items-start space-x-3 rounded-md border p-3">
                ...
                <div className="flex-1">
                  <Label htmlFor="payment-bank" className="flex items-center font-medium cursor-pointer">
                  ...
                  {paymentMethod === "BANK_TRANSFER" && (
                    <div className="mt-2 text-sm text-muted-foreground">
                      ...
                      <p>Reference: Your order number will be provided after checkout</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-start space-x-3 rounded-md border p-3">
                ...
                <div className="flex-1">
                  <Label htmlFor="payment-cod" className="flex items-center font-medium cursor-pointer">
                    <Truck className="mr-2 h-4 w-4" />
                    Cash on Delivery
                  </Label>
                  {paymentMethod === "CASH_ON_DELIVERY" && (
                    <p className="mt-2 text-sm text-muted-foreground">Pay with cash when your order is delivered.</p>
                  )}
                </div>
              </div>
            </RadioGroup>
        ...
        <div className="flex justify-end">
          <Button type="submit" size="lg" disabled={isSubmitting || !selectedAddressId}>
